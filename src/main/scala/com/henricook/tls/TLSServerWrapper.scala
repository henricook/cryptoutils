...
import com.henricook.tls.TLS.CertificateChain
...
import com.henricook.tls.x509.{CertificateVerifier, X509Utils}
...
class TLSServerWrapper(
  ...
  require(
    verifier != null || !clientAuth,
    "No client certificate verifier provided"
  )
  ...
  protected def onClientAuth(clientCertificate: CertificateChain): Unit = {
    val chain: List[TLS.Certificate] =
      clientCertificate.getCertificateList.toList
    if (chain.nonEmpty) {
      onInfo(
        s"Client certificate chain: ${chain.map(_.getSubject).mkString("; ")}"
      )
    }

    if (clientAuth && !verifier.isChainValid(chain)) {
      val message =
        s"Invalid client certificate: ${chain.headOption.fold("<none>")(_.getSubject.toString)}"
  ...
  def apply(connection: SocketChannel): SocketChannel = {
    val protocol = new TlsServerProtocol(
    ...
    val server = new DefaultTlsServer() {
      ...
      private def signerCredentials(
          certOption: Option[TLS.CertificateKey]
      ): TlsSignerCredentials = {
        certOption
          .filter(
            c ⇒
              X509Utils
                .isKeyUsageAllowed(c.certificate, KeyUsage.digitalSignature)
          )
          .fold(throw new TLSException("No suitable signer credentials found")) {
            cert ⇒
              new DefaultTlsSignerCredentials(
                context,
                cert.certificateChain,
                cert.key.getPrivate,
                TLSUtils.signatureAlgorithm(cert.key.getPrivate)
              )
          }
      }

      override def getRSASignerCredentials: TlsSignerCredentials =
        wrapException("Could not provide server RSA credentials") {
          signerCredentials(keySet.rsa)
        }

      override def getECDSASignerCredentials: TlsSignerCredentials =
        wrapException("Could not provide server ECDSA credentials") {
          signerCredentials(keySet.ecdsa)
        }

      override def getDSASignerCredentials: TlsSignerCredentials =
        wrapException("Could not provide server DSA credentials") {
          signerCredentials(keySet.dsa)
        }

      override def getRSAEncryptionCredentials: TlsEncryptionCredentials =
        wrapException("Could not provide server RSA encryption credentials") {
          keySet.rsa
            .filter(
              c ⇒
                X509Utils
                  .isKeyUsageAllowed(c.certificate, KeyUsage.keyEncipherment)
            )
            .fold(super.getRSAEncryptionCredentials) { cert ⇒
      ...
      override def notifyClientCertificate(
          clientCertificate: CertificateChain
      ): Unit = wrapException("Client certificate error") {

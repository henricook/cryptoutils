...
object BCConversions {
  implicit class JavaKeyOps(private val key: java.security.Key) extends AnyVal {
    private def convertPKCS8Key(
        data: Array[Byte],
        public: SubjectPublicKeyInfo
    ): AsymmetricCipherKeyPair = {
      new AsymmetricCipherKeyPair(
        PublicKeyFactory.createKey(public),
        PrivateKeyFactory.createKey(data)
      )
    }

    def toAsymmetricCipherKeyPair(
        public: SubjectPublicKeyInfo
    ): AsymmetricCipherKeyPair = key match {
  ...
  implicit class SubjectPublicKeyInfoOps(private val key: SubjectPublicKeyInfo)
      extends AnyVal {
  ...
  implicit class PrivateKeyInfoOps(private val key: PrivateKeyInfo)
      extends AnyVal {
    def toAsymmetricKeyParameter: AsymmetricKeyParameter = {
  ...
  implicit class JavaKeyPairOps(private val pair: java.security.KeyPair)
      extends AnyVal {
    def toAsymmetricCipherKeyPair: AsymmetricCipherKeyPair = {
      pair.getPrivate.toAsymmetricCipherKeyPair(
        pair.getPublic.toSubjectPublicKeyInfo
      )
    }
  }

  implicit class AsymmetricCipherKeyPairOps(
      private val key: AsymmetricCipherKeyPair
  ) extends AnyVal {
    def toKeyPair: java.security.KeyPair = {
      new java.security.KeyPair(
        key.getPublic.toPublicKey,
        key.getPrivate.toPrivateKey
      )
    }
  }

  implicit class AsymmetricKeyParameterOps(
      private val key: AsymmetricKeyParameter
  ) extends AnyVal {
      ...
      key match {
        ...
        case _ ⇒
          throw new IllegalArgumentException(
            s"Unknown key algorithm: ${key.getClass}"
          )
      }
    }
    ...
    def toPrivateKey: PrivateKey = {
      val keyGenerator =
        KeyFactory.getInstance(this.algorithm, TLSUtils.provider)
      keyGenerator.generatePrivate(
  ...
  implicit class JavaCertificateOps(
  ...
  implicit class CertificateOps(private val cert: TLS.Certificate)
      extends AnyVal {
    def toTlsCertificateChain: TLS.CertificateChain = {
  ...
  implicit class CertificateChainOps(private val chain: TLS.CertificateChain)
      extends AnyVal {
    def toTlsCertificate: TLS.Certificate = {
  ...
  object DigestAlgorithm {
    def apply(name: String): AlgorithmIdentifier = {
      ...
      Option(finder.find(name))
        .getOrElse(
          throw new IllegalArgumentException(
            s"Invalid digest identifier: $name"
          )
        )
    }
  }
  ...
  object CipherSuiteId {
    def apply(cs: String): Int = {
      Try(classOf[CipherSuite].getField(cs).getInt(null))
        .getOrElse(
          throw new IllegalArgumentException(s"Invalid cipher suite: $cs")
        )
    }
    ...
    def asString(cs: Int): String = {
      ...
      fields
        ...
        .fold(throw new IllegalArgumentException(s"Unknown cipher suite: $cs"))(
          _.getName
        )
    }
  }

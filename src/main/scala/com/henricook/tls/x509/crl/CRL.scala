package com.henricook.tls.x509.crl
...
import com.henricook.tls.TLS.CertificateKey
...
import com.henricook.tls.x509.crl.CRLHolder.{
  Revoked,
  RevokedCert,
  RevokedCerts,
  RevokedSerial
...
object CRLHolder {
  ...
  case class RevokedSerial(
      serial: BigInt,
      reason: Int = CRLReason.unspecified,
      revocationDate: Instant = Instant.now()
  ) extends Revoked
  case class RevokedCert(
...
trait CRLReader extends ObjectLoader[X509CRLHolder] {
  ...
  def verify(crl: X509CRLHolder, issuer: TLS.Certificate): Boolean = {
    X509Utils.isKeyUsageAllowed(issuer, KeyUsage.cRLSign) && crl
      .isSignatureValid(X509Utils.contentVerifierProvider(issuer))
  }
...
object CRL extends CRLBuilder with CRLReader {
  private val config =
    ConfigFactory.load().getConfig("henricook.tls.crl-defaults")
  ...
  def defaultNextUpdate(): Instant = {
    Instant
      .now()
      .plus(
        config.getDuration("next-update-in", TimeUnit.SECONDS),
        ChronoUnit.SECONDS
      )
  }

  private def addSerial(
      builder: X509v2CRLBuilder,
      issuer: CertificateKey,
      serial: BigInteger,
      reason: Int,
    ...
    extGen.addExtension(
      ...
      true,
      new GeneralNames(new GeneralName(issuer.certificate.getSubject))
    )
  ...
  def build(
      issuer: TLS.CertificateKey,
      revoked: Seq[Revoked],
      nextUpdate: Instant = defaultNextUpdate()
  ): X509CRLHolder = {
    assert(
      X509Utils.isKeyUsageAllowed(issuer.certificate, KeyUsage.cRLSign),
      "CRL signing not allowed"
    )

    val builder =
      new X509v2CRLBuilder(issuer.certificate.getSubject, new Date())
    val extensionUtils = X509Utils.extensionUtils(defaultKeyIdAlgorithm())
    val contentSigner = X509Utils.contentSigner(
      issuer.key.getPrivate.toPrivateKey,
    ...
    builder.addExtension(
      ...
      extensionUtils.createAuthorityKeyIdentifier(
        new X509CertificateHolder(issuer.certificate)
      )
    )

    ...
    def isChainValidRec(
      ...
      chain match {
        ...
        case cert :: issuer :: rest ⇒
          X509Utils.getPathLengthConstraint(issuer).fold(true)(_ >= position) &&
            isCertificateValid(cert, issuer) && isChainValidRec(
            issuer :: rest,
            position + 1
          )
...
object CertificateVerifier {
  ...
  /**
    ...
    */
  def fromTrustStore(
      trustStore: KeyStore = TrustStore.default(),
    ...
    new DefaultCertificateVerifier(
  ...
  /**
    ...
    * @return Certificate verifier
    */
  def trustAll(): CertificateVerifier =
    new DefaultCertificateVerifier(
      Set.empty,
      CertificateStatusProvider.AlwaysValid
    ) {
...
class DefaultCertificateVerifier(
    override val trustedRootCertificates: Set[TLS.Certificate],
    val certificateStatusProvider: CertificateStatusProvider
) extends CertificateVerifier {
  override def isCertificateValid(
      certificate: TLS.Certificate,
      issuer: TLS.Certificate
  ): Boolean = {
    ...
    X509Utils.isCertificationAuthority(issuer) && X509Utils.isKeyUsageAllowed(
      issuer,
      KeyUsage.keyCertSign
    ) &&
    certHolder.isSignatureValid(contentVerifierProvider) && certHolder
      .isValidOn(new Date()) &&
  ...
  override def isCAValid(certificate: TLS.Certificate): Boolean = {
    // CA authority
    trustedRootCertificates.find(_.getSubject == certificate.getIssuer).exists {
      ca ⇒
        isCertificateValid(certificate, ca) // Verify with stored root CA certificate
    }
  }

  override def isHostValid(
      certificate: TLS.Certificate,
      hostName: String
  ): Boolean = {
    val certHost: String =
      certificate.getSubject.getRDNs(BCStyle.CN).head.getFirst.getValue.toString
    ...
    def check(actual: List[String], cert: List[String]): Boolean = {
      (actual, cert) match {
        ...
        case (actualPart :: actualRest, certPart :: certRest)
            if actualPart.compareToIgnoreCase(certPart) == 0 ⇒
    ...
    def checkIp(hostName: String, ip: ASN1Encodable): Boolean = {
      Try(
        util.Arrays.equals(
          ip.asInstanceOf[DEROctetString].getOctets,
    ...
    val sanCheck = {
      ...
      host.exists(
        dns ⇒
          check(
            asList(dns.toString),
            asList(InetAddress.getByName(hostName).getHostName)
          )
      )
    }

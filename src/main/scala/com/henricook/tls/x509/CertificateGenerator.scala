...
class CertificateGenerator {
  ...
  private def makeChain(
      issuer: TLS.CertificateChain,
  ...
  def createRequest(
    ...
    builder.addAttribute(
      ...
      CSRUtils.encodeExtensions(extensions)
    )
  ...
  def signRequest(
    ...
    (extensions ++ CertExtension.identifiers(
      request.getSubjectPublicKeyInfo,
      Some(issuer.certificate)
    ) ++ CSRUtils.extensionsOf(request)).foreach { ext ⇒
      certificateBuilder.addExtension(ext.id, ext.critical, ext.value)
    }

    makeChain(
      issuer.certificateChain,
      certificateBuilder.build(contentSigner).toASN1Structure
    )
  }
  ...
  def create(
    ...
    val contentSigner = X509Utils.contentSigner(signKey)
    val certificateBuilder = new X509v3CertificateBuilder(
      issuer.fold(subject)(_.certificate.getSubject),
      serial.underlying(),
      new Date(),
      Date.from(notAfter),
      subject,
    ...
    TLS.CertificateKey(
      issuer.fold(certificate.toTlsCertificateChain)(
        is ⇒ makeChain(is.certificateChain, certificate)
      ),
      keyPair.toAsymmetricCipherKeyPair
    )
  }
  ...
  def generate(
    ...
    issuer.foreach { ca ⇒
      assert(
        X509Utils.isCertificationAuthority(ca.certificate) && X509Utils
          .isKeyUsageAllowed(ca.certificate, KeyUsage.keyCertSign),
    ...
    generator.initialize(
      if (size == 0) X509Utils.defaultKeySize(algorithm) else size,
      secureRandom
    )
  ...
  def generateEcdsa(
      subject: X500Name,
      curve: ECParameterSpec = X509Utils.defaultEllipticCurve(),
      issuer: Option[TLS.CertificateKey] = None,
      serial: BigInt = BigInt(1),
      notAfter: Instant = X509Utils.defaultExpire(),
      extensions: Set[CertExtension] = CertExtension.defaultExtensions()
    ...
    val keyPairGenerator =
      KeyPairGenerator.getInstance("ECDSA", TLSUtils.provider)
  ...
  def generateKeySet(
      subject: X500Name,
      rsaSize: Int = X509Utils.defaultKeySize("RSA"),
      dsaSize: Int = X509Utils.defaultKeySize("DSA"),
      curve: ECParameterSpec = X509Utils.defaultEllipticCurve(),
      issuer: Option[TLS.CertificateKey] = None,
      serial: BigInt = BigInt(1),
      notAfter: Instant = X509Utils.defaultExpire(),
    ...
    val rsa =
      generate(subject, "RSA", rsaSize, issuer, serial, notAfter, extensions)
    val dsa =
      generate(subject, "DSA", dsaSize, issuer, serial, notAfter, extensions)
    val ecdsa =
      generateEcdsa(subject, curve, issuer, serial, notAfter, extensions)
